...
func checkTokenizationOfSamples() {
    Tokens.FooterToken
        ...
        .forEach { sample in
            ...
            let token = sample.source.tokenizeReportFooter().first ?? Tokens.FooterToken(source: sample.source, symbol: .error)
...
func compareTokenizationOfSourceFilesWithSamples() throws {
    let samples = Tokens.FooterToken.allFooterTokens.flatMap { $0 }
