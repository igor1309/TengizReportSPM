...
func generateFooterSamples() throws {
    ...
    footerTokens.enumerated()
        .forEach {
            ...
            print("\nstatic var footerTokens2020\(offset): [Tokens.FooterToken] {\n[", terminator: "")
            $0.element.forEach {
//                print("Tokens.\($0),")

                print("Tokens.FooterToken(source: ", terminator: "")
...
func checkFooterSampleSources() throws {
    let footersInSamples = Tokens.FooterToken.allFooterTokens
        .map { token in
            token
                .map(\.source)
                .joined(separator: "\n")
